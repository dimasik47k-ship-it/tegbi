...
const BotCard = ({ bot, onDetailsClick }) => {
  ...
  console.log('Рендеринг карточки бота:', bot.name, 'ID:', bot.id);
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden transform border-2 border-red-500 ${isHovered ? 'scale-105 -translate-y-1' : 'scale-100'} fade-in`}
      onMouseEnter={() => {
        setIsHovered(true);
        playHoverSound();
      }}
