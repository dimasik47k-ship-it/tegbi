...
const BotCard = ({ bot, onDetailsClick, className = '' }) => {
  ...
  return (
    <div
      ...
      className={`bg-white rounded-xl shadow-md hover:shadow-2xl transition-all duration-300 overflow-hidden transform ${isHovered ? 'scale-105 -translate-y-1' : 'scale-100'} fade-in ${className}`}
      onMouseEnter={() => {
        setIsHovered(true);
        playHoverSound();
      }}
