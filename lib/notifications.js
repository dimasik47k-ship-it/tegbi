...
export async function sendAdminNotification(submission) {
  const BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN; // Твой бот для уведомлений
  const ADMIN_CHAT_ID = process.env.ADMIN_CHAT_ID;   // Твой chat_id
...
// ✉️ Email пользователю (когда заявка принята)
export async function sendApprovalEmail(userEmail, submission, botUrl) {
  // Настройки почтового сервиса (Resend / SendGrid / Nodemailer)
